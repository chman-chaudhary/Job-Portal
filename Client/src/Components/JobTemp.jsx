...
export default function JobTemp() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      let response = await axios.get(`http://localhost:3000/job/${params.id}`, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem("token"),
        },
      });
      const { img, username, employerName, isApplicant, isOwner, applicants } =
        response.data;
      if (isApplicant) {
  ...
  const applyJob = () => {
    const sendApplyRqst = async () => {
      let response = await axios.post(
        `http://localhost:3000/job/${job._id}`,
        {},
        {
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
    ...
    <div className="bg-black text-white custom-fonts pt-[92px]">
      <div className="bg-[#353DFF] rounded-[40px] pt-20 pb-20 m-6">
        <div className="container mx-auto min-w-[1140px]">
          <a href={`${job.companyURL}`} target="_blank">
            <h2 className="text-center text-2xl">{job.companyName}</h2>
          </a>
          ...
          <div className="flex justify-center items-center space-x-5 mt-12 mb-2">
            {isOwner ? (
              ...
              <>
                <div onClick={applyJob}>
                  <Button1 text={applyBtnTxt} />
                </div>
                <Button2 text="SAVE" />
              </>
            )}
      ...
      <div className="px-10 py-8">
        <div className="about mx-6 px-20 pb-8">
          ...
          <pre
            className="text-[19px] custom-fonts leading-loose"
      ...
      <div className="px-10">
        ...
        <div className="px-20">
          {!isOwner && (
            <button
              className="px-[75px] py-3 border border-white rounded-lg bg-[#F6F6F6] text-black hover:bg-black hover:text-white font-medium my-8"
              onClick={applyJob}
            >
              {applyBtnTxt}
            </button>
      ...
      {isOwner && (
        <div className="px-10 py-8 ">
          ...
          {applicants.map((applicant, i) => {
            return (
              <Link key={i} to={`/profile/${applicant.username}`}>
                <div className="mx-6 px-20 flex items-center hover:bg-[#05061A] py-3">
                  <img
                    className="h-28 w-28 rounded-full object-cover"
                    src={
                    ...
                    alt="Profile Picture"
                  />
                  <h2 className="font-medium text-4xl ml-16">
